--Phân hệ khách hàng
	...
	CREATE OR ALTER PROCEDURE KH_XEM_DS_SP
	AS 
		BEGIN TRAN
			SELECT *
			FROM SANPHAM
	...
	GO
	--Tìm kiếm sản phẩm
	CREATE OR ALTER PROCEDURE KH_TIM_SP
	(
		@TENSP NVARCHAR(50)
	)
	AS 
...
--Phân hệ nhân viên
	...
	GO
	--Xem danh sách sản phẩm
	CREATE OR ALTER PROCEDURE NV_XEM_DS_SP
	AS 
		BEGIN TRAN
			SELECT *
			FROM SANPHAM
	...
	GO
	--Tìm kiếm sản phẩm
	CREATE OR ALTER PROCEDURE NV_TIM_SP
	(
		@TENSP NVARCHAR(50)
	)
	AS 
...
--Phân hệ nhân viên giao hàng
	...
	AS 
		BEGIN TRAN
			UPDATE DONGH_KHACH SET MaNVGH = @MANVGH 
								WHERE MaDGHK = @MADGHK
		COMMIT TRAN
	GO
...
--Phân hệ quản trị
	...
	GO
	--Sửa phiếu đặt hàng
	CREATE OR ALTER PROCEDURE QT_SUA_PHIEU_DH
	(
		@MAPDH NVARCHAR(10),
		@NGAYDATHANG DATE,
		@MANCC NVARCHAR(10)
	...
	AS 
		BEGIN TRAN
			UPDATE PHIEUDATHANG SET NgayDatHang = @NGAYDATHANG,
									MaNCC = @MANCC
								WHERE MaPDH = @MAPDH
		COMMIT TRAN
	GO
	--Xóa phiếu đặt hàng
	CREATE OR ALTER PROCEDURE QT_XOA_PHIEU_DH
	(
		@MAPDH NVARCHAR(10)
	)
	AS 
		BEGIN TRAN
			 DELETE FROM PHIEUDATHANG WHERE MaPDH = @MAPDH		
		COMMIT TRAN
	GO
